...
The mean of steps per day is `r format(meansteps,digits = 1)` and the median is `r mediansteps`.
...
The 5-minutes interval with higher activity during the day is `r MaxStepInterval` in average.
...
The total missing values are `r length(NAPos)`. The mean of steps per day is `r format(meansteps_NNA,digits = 1)` and the median is `r format(mediansteps_NNA,digits = 1)`. There is an insignificant change in the median, and no modification in the calculated mean after the NA replacement.
